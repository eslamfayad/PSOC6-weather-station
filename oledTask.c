...
void ShowStartupScreen(void)
{
    ...
    GUI_DispStringAt("Weather Station", 64, 42);
}
...
void oledTask(void *arg)
{
   ...
   /// CY_ASSERT(result == CY_RSLT_SUCCESS);
    ...
    GUI_DispStringAt("Temperature", 30, 3); //GUI_DispString("Temperature:");
    GUI_DispStringAt("Humidity", 42, 32);

    for (;;)
    {

    	uint8_t adc_result = adc_result_0/10;
    	    	         itoa(adc_result,buffer,10);
    	    	         GUI_GotoXY(64, 17);

    	    	         GUI_DispString(buffer);   // for Temperature
    	    	         GUI_GotoXY(64, 50);
    	    	         GUI_DispString(buffer2); //for humidity
    	    	         vTaskDelay(1000);
